...
/* Programma da mandare sulla shield wi-fi ESP8266 ESP-12E.
 *  Questo programma instaura prima di tutto una connessione wi-fi ad una rete WLAN locale
 *  e comunica sia con il dispositivo connesso e allocato all'indirizzo IP della shield sia con gli altri nodi
 *  adiacenti attraverso la connessione ottica.
 *  La pagina HTML che sarà mandata al client presenta diversi radio button per far comunicare
 *  i cambiamenti dei dispositivi posti sui nodi dal Nodo Master.
 *  Il Nodo Master è l'unico nodo che è connesso alla rete domestica ed è a capo del ponte di nodi.
 *  Da lui passano tutti i dati(andata e ritorno che siano).
 *  
 *  La relazione del seguente progetto è presente su GitHub a questo link:
 ...
 *      NETWORK-NODES
 *      
 *      2017 Universitrà politecnica delle Marche
 *      
 *      DATA ULTIMA MODIFICA: SABATO 22 APRILE 2017 ORE 16:01
 *      
 *      */
...
//Verifichiamo se la connessione è la ricezione del messaggio di ACK è avvenuta senza problemi
...
//Variabile contatore per verificare il tempo di attesa dell'invio del messaggio fino alla ricezione dell'ACK
int i = 0;
...
//Quando le variabili sono a TRUE il cambiamento è avvenuto mentre a FALSE il cambiamento potrebbe non essere stato effettuato
  boolean CRN1 = true;
  boolean CAN1 = true;
  boolean CRN2 = true;
  boolean CAN2 = true;
...
{  
  ...
  if (!cliente) 
  {
    //Se la connessione al client non è presente, il Nodo Master è in ascolto verso il Nodo 1 per la ricezione 
  ...
  String form ="";  //Stringa contenente l'indirizzo IP, e eventuale form, del Nodo Master
  ...
  analogic_input = false; //Resettiamo la variabile del controllo dell'input analogico
    ...
    {int conta = 0;
      ...
      do
      {
        ...
        if(i<1000)
        {
          if(conta == 15)          
          {
            irsend.sendNEC(0x0000011000, 32); // Invio dato al nodo 1 per lo spegnimento dell'attuatore nodo 1
            conta = 0;
          }
          comunicazione();
          i++;
    ...
    Serial.println("SPEGNIMENTO RELAY NODO 2");
      do
      {
        ...
        if(i<1000)
        {
          if(conta == 15)          
          {
            irsend.sendNEC(0x0000010000, 32); // Invio dato al nodo 1 per lo spegnimento dell'attuatore nodo 1
            conta = 0;
          }          
          comunicazione();
          i++;
    ...
    Serial.println("ACCENSIONE ATTUATORE NODO 2");
      do
      {
        ...
        if(i<1000)
        {
          if(conta == 15)          
          {
            irsend.sendNEC(0x0001111000, 32); // Invio dato al nodo 1 per lo spegnimento dell'attuatore nodo 1
            conta = 0;
          }          
          comunicazione();
          i++;
    ...
    Serial.println("SPEGNIMENTO ATTUATORE NODO 2");
      do
      {
        ...
        if(i<1000)
        {
          if(conta == 15)          
          {
            irsend.sendNEC(0x0011100000, 32); // Invio dato al nodo 1 per lo spegnimento dell'attuatore nodo 1
            conta = 0;
          }
